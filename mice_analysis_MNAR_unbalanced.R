...
type <- "MNAR_unbalanced"

# whole
for (i in c(10, 20, 30, 40, 50, 60)) {
  # read in data
  df <- read.csv(paste0(type,"/data_clean_mice", i, ".csv"))
...
# write out table
write.csv(missing_level_df, paste0(type,"/mice.csv"), row.names=FALSE) 
...
for (i in c(10, 20, 30, 40, 50, 60)) {
  # read in data
  df <- read.csv(paste0(type,"/data_clean_mice_reduced", i, ".csv"))
  ...
  # use matchit to match
  match_out <- MatchIt::matchit(enhanced_guidelines_yn ~ gender +
                                conservative +
                                education +
                                today_concentration_already_occured +
                                burning_oil_co2
...
# write out table
write.csv(missing_level_df, paste0(type,"/mice_reduced.csv"), row.names=FALSE) 
