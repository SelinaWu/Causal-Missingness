...
for (percent in c("10", "20", "30", "40", "50", "60")) {
  data <- read.csv(paste0("MNAR/data_clean_MNAR_missing_", percent,".csv"), stringsAsFactors = FALSE)
  ...
  write.csv(complete_df, paste0("MNAR/data_clean_mimp", percent,".csv"), row.names=FALSE)
}
...
for (percent in c("10", "20", "30", "40", "50", "60")) {
  ...
  complete_df <- bind_cols(covariates_filled, experimental_variables)
  complete_df <- bind_cols(complete_df, s_cols_original)
  write.csv(complete_df, paste0("MNAR/data_clean_mimp_reduced", percent,".csv"), row.names=FALSE)
}
